...
class AccountApi(RestClient):
    ...
    async def get_v1_account(self, validate_response=True, **kwargs):
        ...
        :return:
        """
        response = await self.get(path=f"/v1/account", **kwargs)
        if validate_response:
            return UserDetailsEnvelope(**response.json())
        return response

    @allure.step("Сброс пароля")
    async def post_v1_account_password(self, **kwargs):
        """
        Reset registered user password
        :return:
        """
        response = await self.post(path=f"/v1/account/password", **kwargs)
        UserEnvelop(**response.json())
        return response

    @allure.step("Изменения пароля")
    async def put_v1_account_password(self, **kwargs):
        """
        Change registered user password
        :return:
        """
        response = await self.put(path=f"/v1/account/password", **kwargs)
        return UserEnvelop(**response.json())

    @allure.step("Активация пользователя")
    async def put_v1_account_token(self, user_token, validate_response=True):
        """
    ...
    async def put_v1_account_email(self, json_data, validate_response=True):
        """
        ...
        :return:
        """
        response = await self.put(path=f"/v1/account/email", json=json_data)
        if validate_response:
            return UserEnvelop(**response.json())
        return response
